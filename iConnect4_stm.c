...
static const adc adc_parameters[] = {
		...
		{&hadc2, ADC_CHANNEL_12},
		{&hadc1, ADC_CHANNEL_12},
		{&hadc2, ADC_CHANNEL_15},
		{&hadc1, ADC_CHANNEL_5},
		{&hadc3, ADC_CHANNEL_5},
		{&hadc2, ADC_CHANNEL_5}
};
...
int main(void)
{
  ...
  for (int k = 0; k < num_servos; k++) HAL_TIM_PWM_Start(tim_parameters[k].htim,  tim_parameters[k].channel);
	...
	else if (!first_move)
	{
		for (int k = 1; k < num_sensors; k++)
		{
			if (cal[k] > ((float)ldr[k] / (float)ref[k]) + threshold)
			{
				...
				for (;;) // verify algorithm move made it to column
				{
					ldr[alg_column_num[0] - '0'] = ADC_Convert(adc_parameters[alg_column_num[0] - '0'].hadc, adc_parameters[alg_column_num[0] - '0'].channel, ADC_SAMPLETIME_247CYCLES_5);
					if (cal[alg_column_num[0] - '0'] > ((float)ldr[alg_column_num[0] - '0'] / (float)ref[alg_column_num[0] - '0']) + threshold)
					{
						printf("confirmed algorithm move!\r\n");
						HAL_Delay(2000);
						// open all servos for player move
						for (int k = 0; k < num_servos; k++) __HAL_TIM_SET_COMPARE(tim_parameters[k].htim, tim_parameters[k].channel, 1000);
						break;
					}
					HAL_Delay(100);
